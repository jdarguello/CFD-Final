...
def Solution(datos, mesh, size=(20,5)):
    fig = plt.figure(figsize=size)
    T = datos['Fluido']['Tiempo final [s]']            # final time
    num_steps = 100   # number of time steps
    #dt = T / num_steps # time step size
    dt = datos['Fluido']['Diferencial de tiempo [s]']
    num_steps = int(T/dt)
    mu = datos['Fluido']['Viscosidad']         # dynamic viscosity
    rho = datos['Fluido']['Densidad [kg/m3]']/997            # density
    ...
    inflow, outflow, walls, cylinder = BC(datos['Geo'])
